...
d95.du <- ggj.dissimilarity_unit(d95,"Gender","OccMinor", weight="value")
d00.du <- ggj.dissimilarity_unit(d00,"Gender","OccMinor", weight="value")
d05.du <- ggj.dissimilarity_unit(d05,"Gender","OccMinor", weight="value")

#' prepare tables for decomposition
d85.dm <- ggj.prepare_data(d85,"Gender","OccMinor", weight="value")
d85.dm[, n_group := sum(freq), by = Gender]
d85.dm[, n_unit := sum(freq), by = OccMinor]
d85.dm[, n_all := sum(freq)]

d90.dm <- ggj.prepare_data(d90,"Gender","OccMinor", weight="value")
d90.dm[, n_group := sum(freq), by = Gender]
d90.dm[, n_unit := sum(freq), by = OccMinor]
d90.dm[, n_all := sum(freq)]

d95.dm <- ggj.prepare_data(d95,"Gender","OccMinor", weight="value")
d95.dm[, n_group := sum(freq), by = Gender]
d95.dm[, n_unit := sum(freq), by = OccMinor]
d95.dm[, n_all := sum(freq)]

d00.dm <- ggj.prepare_data(d00,"Gender","OccMinor", weight="value")
d00.dm[, n_group := sum(freq), by = Gender]
d00.dm[, n_unit := sum(freq), by = OccMinor]
d00.dm[, n_all := sum(freq)]

d05.dm <- ggj.prepare_data(d05,"Gender","OccMinor", weight="value")
